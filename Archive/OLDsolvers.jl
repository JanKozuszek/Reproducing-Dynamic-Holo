...
function CorrectXi(Var, X, t, margin::Int64 = 10)
    ...
    for ii in (margin+1):N
        if ii%Npts == 1
            ...
            SdotSub = Var[3, ii];
            push!(subgrid, z);
            push!(SdotFull,UnSubSdot(SdotSub,Xnew,z,t)+val_const);
        end
    end 

    interpolant = poly.fit(subgrid, SdotFull, 20);
    #Try something different with the horizon - move it off of zAH, keep Sdot = -cnst there instead.
    ...
    Xnew = Xnew - 1. /(2. * zAH) + 1. /(2. * sol.u) - val_const*(zAH - sol.u);
